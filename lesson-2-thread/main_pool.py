...
def get_m3u8_url(url):
    ...
    head_name = soup.find("div",class_="ranking-item-info").find("a", class_="text-overflow").string
    chapter_name = soup.find("ul", class_="ewave-playlist-content").find("li",class_="active").string
    script_tag = soup.find("div", class_="embed-responsive").find("script",attrs={"type":"text/javascript"}).string
    # print(head_name, chapter_name, script_tag)
    head_name = head_name + "-" + chapter_name
...
def download_one_ts(i,one_ts,save_dir):
...
def get_ts_file(ts_list,head_name):
    thread_num = 8
    # save_dir = "cache_ts"
    save_dir = head_name
    ...
    with ThreadPoolExecutor(max_workers=thread_num) as exe:
        for i, one_ts in enumerate(ts_list):
            downloaded_file = exe.submit(download_one_ts,i,one_ts,save_dir)
            dd_dict[downloaded_file] = i

        for dd in as_completed(dd_dict):
            idx = dd_dict[dd]
            try:
                res = dd.result()
                local_file[idx] = res
...
if __name__ == '__main__':
    ...
    file_list = get_ts_file(ts_list,head_name)
    print(file_list)
